    ...
    <div class="container mt-5">
        ...
        <table id="parcelTable" class="table table-striped">
            ...
            <tbody>
                ...
                $sql = "SELECT * FROM form_submissions WHERE receiver = 'งานพัสดุ' AND continuously_status = 'รับแล้ว' ORDER BY id DESC";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>" . $row['parcel_data'] . "</td>";
                        echo "<td>" . $row['reference_number'] . "</td>";
                        echo "<td>";
                        if ($row['parcel_status'] === 'แก้ไขงานพัสดุ') {
                            echo '<i class="bi bi-lightbulb blinking-icon"></i> ' . $row['parcel_status'];
                        } else {
                            echo $row['parcel_status'];
                        }
                        echo "</td>";
                        echo "<td>";
                        if (empty($row['parcel_status'])) {
                            echo "<button class='receiveBtn btn btn-success'><i class='bi bi-receipt'></i> รับเอกสาร</button>";
                        } elseif ($row['parcel_status'] === 'รับแล้ว') {
                            echo "<button class='editBtn btn btn btn-danger'><i class='bi bi-pencil'></i> แก้ไข</button>";
                        } elseif ($row['parcel_status'] === 'แก้ไขงานพัสดุ') {
                            echo "<button class='receiveBtn btn btn-success'><i class='bi bi-receipt'></i> รับเอกสาร</button>";
                        }
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response === 'success') {
                                    ...
                                    sendAjaxRequest('edit', referenceNumber, 'แก้ไขงานพัสดุ');
                                    let row = $(`td:contains(${referenceNumber})`).closest('tr');
                                    row.find('.editBtn')
