...
<head>
    <title>เอกสารลงรับ งานต่อเนื่อง</title>
...
<body>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>" . $row['continuously_data'] . "</td>";
                        echo "<td>" . $row['reference_number'] . "</td>";
                        echo "<td>";
                        if ($row['continuously_status'] === 'แก้ไขงานต่อเนื่อง') {
                            echo '<i class="bi bi-lightbulb blinking-icon"></i> ' . $row['continuously_status'];
                        } else {
                            echo $row['continuously_status'];
                        }
                        echo "</td>";
                        echo "<td>";
    ...
    <script>
        $(document).ready(function() {
            ...
            function sendAjaxRequest(action, referenceNumber, status = null) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response === 'success') {
                                    ...
                                    sendAjaxRequest('edit', referenceNumber, 'แก้ไขงานต่อเนื่อง');
                                    let row = $(`td:contains(${referenceNumber})`).closest('tr');
                                    row.find('.editBtn')
