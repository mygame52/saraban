...
if (isset($_POST['reference_number']) && isset($_POST['action'])) {
    ...
    switch ($action) {
        ...
        case 'edit':
            updateDocumentStatus($conn, $referenceNumber, 'แก้ไขงานการเงิน', $currentDate);
            break;
...
function updateDocumentStatus($conn, $referenceNumber, $status, $date) {
    $statusValue = ($status === 'รับแล้ว') ? 5 : 6; // กำหนดค่า statusValue เป็น 3 หรือ 4 ตามสถานะ

    $sql = "UPDATE form_submissions SET finance_status = '$status', finance_data = '$date' WHERE reference_number = '$referenceNumber'";
    if ($conn->query($sql) === TRUE) {
        // อัปเดตค่า status ให้เป็น 5 หรือ 6 ตามสถานะ
